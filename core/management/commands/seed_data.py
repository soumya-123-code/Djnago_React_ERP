...
class Command(BaseCommand):
        ...
        for i, vendor in enumerate(vendors):
            ...
            warehouse, created = Warehouse.objects.get_or_create(
                ...
                defaults={
                    ...
                    'contact_person': vendor.user.get_full_name(),
    ...
    def create_inventory(self, products, warehouses):
        ...
        self.stdout.write(f'  Created inventory for {len(products)} products')
        ...
        for vendor in vendors:
            ...
            for i in range(2):  # 2 POs per vendor
                ...
                if created:
                    ...
                    po.tax_amount = subtotal * Decimal('0.18')
    ...
    def create_sales_orders(self, customers, products, vendors):
        ...
        for i, customer in enumerate(customers):
            address = CustomerAddress.objects.filter(customer=customer, address_type='shipping').first()
            if not address:
                continue
            ...
            for j in range(2):  # 2 orders per customer
                ...
                if created:
                    ...
                    so.tax_amount = subtotal * Decimal('0.18')
                    so.total_amount = subtotal + so.tax_amount + so.shipping_amount
                    so.save()
    ...
    def create_payments(self, sales_orders):
        ...
        self.stdout.write(f'  Created payments for paid orders')
        ...
        for user in delivery_users:
            agent, created = DeliveryAgent.objects.get_or_create(
                ...
                defaults={
                    ...
                    'license_number': f'MH{random.randint(10, 50)}{random.randint(10000000, 99999999)}',
                    'status': 'available',
                    'is_verified': True,
                    'current_city': random.choice(['Mumbai', 'Delhi', 'Bangalore']),
                    'rating': Decimal(str(round(random.uniform(3.5, 5.0), 2))),
            ...
            if delivery_agents:
                ...
                DeliveryAssignment.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        'assigned_at': timezone.now() - timezone.timedelta(days=random.randint(1, 5)),
                        'estimated_delivery': timezone.now() + timezone.timedelta(days=random.randint(1, 3)),
                    }
                )

        self.stdout.write(f'  Created delivery assignments')
    ...
    def create_notifications(self, users):
        ...
        # Create templates
        templates = [
            {'name': 'order_confirmed', 'title': 'Order Confirmed', 'body': 'Your order {{order_number}} has been confirmed.'},
            {'name': 'order_shipped', 'title': 'Order Shipped', 'body': 'Your order {{order_number}} has been shipped.'},
            {'name': 'order_delivered', 'title': 'Order Delivered', 'body': 'Your order {{order_number}} has been delivered.'},
            {'name': 'welcome', 'title': 'Welcome!', 'body': 'Welcome to our platform, {{user_name}}!'},
        ]
        ...
        notification_types = ['order_update', 'promotion', 'system']
        for user in users[:5]:
            for i in range(3):
